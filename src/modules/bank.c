...
static int module_InitMultiDynamic (module_bank_t *bank, const char *path,
                                    bool fast, const char *relpath, const struct stat *st);			// sunqueen add
...
//        module = module_InitDynamic (bank->obj, abspath, true);
        return module_InitMultiDynamic (bank, abspath, true, relpath, st);			// sunqueen modify
//    if (module == NULL)
//        return -1;
...
// sunqueen add start
static const char loopPszName[][50] = {
										"access_alsa",
										"access_attachment",
										"access_avio",
										"libbluray",
 ...
 */
static int module_InitMultiDynamic (module_bank_t *bank, const char *path,
                                    bool fast, const char *relpath, const struct stat *st)
{
    module_handle_t handle;
	int	workFlag, loadCnt = 0;
	char entry_name[512];

    if (module_Load (bank->obj, path, &handle, fast))
        return NULL;

	for (workFlag = 0; strlen(loopPszName[workFlag]); workFlag++) {
		sprintf(entry_name, "vlc_entry__%s", loopPszName[workFlag]);

	    vlc_plugin_cb entry =
	        (vlc_plugin_cb) module_Lookup (handle, entry_name);
	    if (entry == NULL)
	    {
	        msg_Warn (bank->obj, "cannot find plug-in entry point in %s", path);
			continue;
	    }
	    ...
	    if (unlikely(module == NULL))
	    {
	        ...
	        msg_Err (bank->obj, "cannot initialize plug-in %s", path);
	        break;
	    }
