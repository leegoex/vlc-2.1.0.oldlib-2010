...
vlc_module_begin ( gnutls )				// sunqueen modify
...
static int gnutls_CertSearch (vlc_tls_t *obj, const char *host,
                              const char *service,
                              const gnutls_datum_t *__restrict datum)			// sunqueen modify
{
    assert (host != NULL);
                               ...
                               int type, int fd)
{
    ...
    int val = gnutls_init (&sys->session, (gnutls_connection_end_t)type);			// sunqueen modify
    if (val != 0)
    {
