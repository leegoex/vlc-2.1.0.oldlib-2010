// sunqueen rename events.c to events.hh
/*****************************************************************************
...
#include <vlc_keys.h>
//#include "common.h"			// sunqueen delete
...
static void EventThreadMouseHide( event_thread_t *p_event )				// sunqueen modify
{
    PostMessage( p_event->hwnd, WM_VLC_HIDE_MOUSE, 0, 0 );
}

static void EventThreadUpdateTitle( event_thread_t *p_event, const char *psz_fallback )				// sunqueen modify
{
...
static int EventThreadGetWindowStyle( event_thread_t *p_event )				// sunqueen modify
{
    /* No need to lock, it is serialized by EventThreadStart */
    return p_event->i_window_style;
}

static void EventThreadUpdateWindowPosition( event_thread_t *p_event,
                                      bool *pb_moved, bool *pb_resized,
                                      int x, int y, unsigned w, unsigned h )				// sunqueen modify
{
    vlc_mutex_lock( &p_event->lock );
...
static void EventThreadUpdateSourceAndPlace( event_thread_t *p_event,
                                      const video_format_t *p_source,
                                      const vout_display_place_t *p_place )				// sunqueen modify
{
    vlc_mutex_lock( &p_event->lock );
...
static void EventThreadUseOverlay( event_thread_t *p_event, bool b_used )				// sunqueen modify
{
    vlc_mutex_lock( &p_event->lock );
    p_event->use_overlay = b_used;
    vlc_mutex_unlock( &p_event->lock );
}
static bool EventThreadGetAndResetHasMoved( event_thread_t *p_event )				// sunqueen modify
{
    vlc_mutex_lock( &p_event->lock );
...
static event_thread_t *EventThreadCreate( vout_display_t *vd)				// sunqueen modify
{
...
static void EventThreadDestroy( event_thread_t *p_event )				// sunqueen modify
{
    free( p_event->psz_title );
...
static int EventThreadStart( event_thread_t *p_event, event_hwnd_t *p_hwnd, const event_cfg_t *p_cfg )				// sunqueen modify
{
...
static void EventThreadStop( event_thread_t *p_event )				// sunqueen modify
{
    if( !p_event->b_ready )
        return;
